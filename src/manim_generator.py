...
11. NEVER use MathTex, Tex, or any LaTeX-based rendering (they require LaTeX binary to be installed on the host system, which is not available). If you need math symbols or equations, write them in standard Text or Paragraph objects (e.g. Text('C = 2 * pi * r') or Text('π')).
        ...
        try:
            ...
            if provider == 'openai':
                ...
                response = client.chat.completions.create(
                    model=model,
                    messages=[
                        {"role": "system", "content": "You are an expert in Manim Community Edition (v0.19.1). You generate simple, functional Python code without errors. NEVER use self.camera.frame in Scene. NEVER use MathTex, Tex, or any LaTeX-based rendering. Always respond in valid JSON format."},
                ...
                response = client.messages.create(
                    model=model,
                    max_tokens=4000,
                    system="You are an expert in Manim Community Edition (v0.19.1). You generate simple, functional Python code without errors. NEVER use self.camera.frame in Scene. NEVER use MathTex, Tex, or any LaTeX-based rendering. Always respond in valid JSON format.",
                    messages=[
                        {"role": "user", "content": prompt}
                    ]
                )
                ...
                response = client.models.generate_content(
                    ...
                    config=types.GenerateContentConfig(
                        system_instruction="You are an expert in Manim Community Edition (v0.19.1). You generate simple, functional Python code without errors. NEVER use self.camera.frame in Scene. NEVER use MathTex, Tex, or any LaTeX-based rendering. Always respond in valid JSON format.",
...
9. NEVER use MathTex, Tex, or any LaTeX-based rendering (they are not supported on the host system). If the error relates to MathTex, Tex, or LaTeX rendering, immediately convert the equation/symbols to standard Text or Paragraph objects (e.g. Text('C = 2 * pi * r') or Text('π')).
        ...
        try:
            if provider == 'openai':
                ...
                response = client.models.generate_content(
                    ...
                    contents=fix_prompt,
                    config=types.GenerateContentConfig(
                        system_instruction="You are an expert debugger for Manim Community Edition (v0.19.1). You fix Python code errors. Always respond in valid JSON format.",
                        response_mime_type="application/json"
                    )
                )
                response_text = response.text.strip()
